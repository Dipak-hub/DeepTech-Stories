...
const VideoWrapper_1 = require("./VideoWrapper");
const UnifiedLinearGradient_1 = require("./UnifiedLinearGradient");
const UnifiedIcon_1 = require("./UnifiedIcon");
const haptics_1 = require("../utils/haptics");
const { width: W, height: H } = react_native_1.Dimensions.get('window');
const DEFAULT_DUR = 5000;
function StoryVideo({ url, paused, onLoad }) {
    return (<VideoWrapper_1.VideoWrapper url={url} paused={paused} onLoad={onLoad} style={react_native_1.StyleSheet.absoluteFill}/>);
}
function AnimatedUserStory({ user, storyIndex, i, scrollX, getUserStories, getStoryMediaUrl, getStoryMediaType, onLoad, animationType, paused, }) {
    ...
    return (<react_native_reanimated_1.default.View style={[react_native_1.StyleSheet.absoluteFill, animatedStyle]} pointerEvents="none">
      {isVideo ? (<StoryVideo url={getStoryMediaUrl(story)} paused={paused} onLoad={onLoad}/>) : (<react_native_1.Image source={{ uri: getStoryMediaUrl(story) }} style={react_native_1.StyleSheet.absoluteFill} resizeMode="cover" onLoad={() => {
                setTimeout(onLoad, 50);
            }}/>)}
      <UnifiedLinearGradient_1.UnifiedLinearGradient colors={['rgba(0,0,0,0.6)', 'transparent']} style={styles.scrimTop} pointerEvents="none"/>
      <UnifiedLinearGradient_1.UnifiedLinearGradient colors={['transparent', 'rgba(0,0,0,0.8)']} style={styles.scrimBottom} pointerEvents="none"/>
    </react_native_reanimated_1.default.View>);
}
function StoryViewer({ users, initialUserId, visible, onClose, viewedStoryIds = [], onMarkViewed, onStoryOpen, onStoryView, onStoryClose, onAllStoriesViewed, getUserId, getUserAvatarUrl, getUserName, getUserStories, getStoryId, getStoryMediaUrl, getStoryMediaType, getStoryDuration, defaultStoryDuration = 5000, maxVideoDuration = 30000, animationType = 'cube', showsReplyInput = true, showsOptionsIcon = true, showsLikeIcon = true, showsShareIcon = true, onReplySubmit, onLikePress, onSharePress, onOptionsPress, headerStyle, usernameStyle, timestampStyle, footerStyle, replyInputStyle, progressBarTrackStyle, progressBarFillStyle, renderHeader, renderFooter, renderCaption, }) {
    ...
    const pan = react_native_gesture_handler_1.Gesture.Pan()
        ...
        scrollX.value = (0, react_native_reanimated_1.withSpring)(nextIdx * W, {
            damping: 20,
            stiffness: 250,
            mass: 0.8,
        });
    ...
    const handleLike = (0, react_1.useCallback)(() => {
        if (!activeUser || !activeStory)
            return;
        (0, haptics_1.triggerHaptic)(haptics_1.HapticImpactStyle.Light);
      ...
      <react_native_1.View style={styles.root}>
        <react_native_gesture_handler_1.GestureDetector gesture={gesture}>
          <react_native_1.View style={react_native_1.StyleSheet.absoluteFill}>
            {users.map((u, i) => (<AnimatedUserStory key={getUserId(u)} user={u} storyIndex={storyIndices[getUserId(u)] || 0} i={i} scrollX={scrollX} getUserStories={getUserStories} getStoryMediaUrl={getStoryMediaUrl} getStoryMediaType={getStoryMediaType} animationType={animationType} paused={paused || i !== userIndex} onLoad={() => {
                if (i === userIndex) {
        ...
        {renderHeader ? (<react_native_1.View style={[styles.customHeaderWrap, { top: insets.top + 22 }]} pointerEvents="box-none">
            ...
            <react_native_1.View style={{ flexDirection: 'row', alignItems: 'center' }}>
              {showsOptionsIcon && (<react_native_1.Pressable onPress={handleOptions} style={{ padding: 8, marginRight: 4 }}>
                  <UnifiedIcon_1.UnifiedIcon family="Ionicons" name="ellipsis-vertical" size={20} color="#fff"/>
                </react_native_1.Pressable>)}
              <react_native_1.Pressable onPress={onClose} hitSlop={14} style={{ padding: 4 }}>
                <UnifiedIcon_1.UnifiedIcon family="Ionicons" name="close" size={28} color="#fff"/>
              </react_native_1.Pressable>
            </react_native_1.View>
          </react_native_1.View>)}
        ...
        <react_native_1.KeyboardAvoidingView behavior={react_native_1.Platform.OS === 'ios' ? 'padding' : undefined} style={styles.footerContainer} pointerEvents="box-none">
          {renderFooter ? (renderFooter({ user: activeUser, story: activeStory, onReply: setReplyText })) : (<>
              <react_native_1.View style={styles.viewersWrap}>
                <UnifiedIcon_1.UnifiedIcon family="Feather" name="eye" size={12} color="rgba(255,255,255,0.8)"/>
              ...
              <react_native_1.View style={[styles.footer, footerStyle, { paddingBottom: Math.max(insets.bottom, 16) }]}>
                ...
                {showsLikeIcon && (<react_native_1.Pressable onPress={handleLike} style={styles.iconBtn}>
                    <UnifiedIcon_1.UnifiedIcon family="Ionicons" name="heart-outline" size={28} color="#fff"/>
                  </react_native_1.Pressable>)}
                {showsShareIcon && (<react_native_1.Pressable onPress={handleShare} style={styles.iconBtn}>
                    <UnifiedIcon_1.UnifiedIcon family="Feather" name="send" size={24} color="#fff"/>
                  </react_native_1.Pressable>)}
              </react_native_1.View>
            </>)}
